...
const aggregatePlayerBattingStats = (teamStats: TeamStats, team: Team) => {
  ...
  team.players.forEach(player => {
    ...
    let playerStats = teamStats.playerStats.find(ps => ps.playerId === player.id);
    ...
    // この選手の打席を抽出
    const playerAtBats = team.atBats.filter(atBat => atBat.playerId === player.id);
    
    // 打点の合計
    playerStats.rbis += playerAtBats.reduce((sum, atBat) => sum + (atBat.rbi || 0), 0);
    ...
    // 打率計算 (打数が0の場合は0)
    playerStats.battingAvg = playerStats.atBats > 0 ? playerStats.hits / playerStats.atBats : 0;
    
    // 出塁率計算 (打数+四球が0の場合は0)
    const plateAppearances = playerStats.atBats + playerStats.walks;
    playerStats.obp = plateAppearances > 0 ? (playerStats.hits + playerStats.walks) / plateAppearances : 0;
    
    // 長打率計算 (打数が0の場合は0)
    const totalBases = playerStats.singles + (playerStats.doubles * 2) + (playerStats.triples * 3) + (playerStats.homeRuns * 4);
    playerStats.slg = playerStats.atBats > 0 ? totalBases / playerStats.atBats : 0;
